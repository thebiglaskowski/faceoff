...
def filter_faces_by_confidence(faces, threshold: float = None):
    ...
    filtered = [
        f
        for f in faces
        if (f.det_score if hasattr(f, "det_score") else 1.0) >= threshold
    ]
...
class FaceProcessor:
    ...
    def __init__(self, device_id: int = 0, confidence: float = None):
        ...
        self.confidence = (
            confidence if confidence is not None else config.face_confidence_threshold
        )
        ...
        try:
            ...
            for i, face in enumerate(faces, 1):
                ...
                gender = (
                    ...
                    else "Female" if hasattr(face, "gender") else "Unknown"
                )
                info_lines.append(
    ...
    def detect_and_extract_faces(
        ...
        try:
            ...
            info_text = (
                f"✅ Detected {len(faces)} face(s) (sorted left→right, top→bottom)"
            )
    ...
    def get_display_text(self) -> str:
        ...
        with self._lock:
            if not self._mappings:
                return "No mappings"
            return "\n".join(
                [f"Source Face {s} → Target Face {t}" for s, t in self._mappings]
            )

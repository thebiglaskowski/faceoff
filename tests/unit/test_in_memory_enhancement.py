...
from processing.in_memory_enhancement import (
    InMemoryEnhancer,
    resolve_enhancement_device_ids,
)
...
class TestResolveEnhancementDevices:
    ...
    def test_realesrgan_respects_multi_gpu_config(self, temp_config):
        with (
            patch("processing.in_memory_enhancement.config") as mock_cfg,
            patch(
                "processing.in_memory_enhancement.select_enhancement_gpu",
                return_value=0,
            ),
        ):
...
class TestInMemoryEnhancer:
    ...
    def test_multi_gpu_splits_frames(self):
        ...
        with patch.object(
            enhancer, "_enhance_bgr_batch", side_effect=lambda bgr, gid: bgr
        ):

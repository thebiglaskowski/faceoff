...
class FriendlyError(FaceOffError):
    ...
    def format_message(self) -> str:
        ...
        lines = [f"❌ {self.title}", "━" * 60, self.message, ""]
...
class ErrorHandler:
    ...
    def handle_error(error: Exception, context: dict = None) -> FriendlyError:
        ...
        if "model" in error_str and (
            "failed" in error_str or "initialization" in error_str
        ):
        ...
        if error_type in ("FileNotFoundError",) and context.get("media_type") in (
            "video",
            "gif",
        ):
            return ErrorHandler._handle_media_error(error, context)
    ...
    def _handle_oom_error(context: dict) -> FriendlyError:
        ...
        suggestions = [
            ...
            (
                f"Lower output scale: {outscale}x → {outscale//2}x"
                if outscale > 2
                else "Lower output scale to 2x"
            ),
        ]
    ...
    def _handle_no_faces_error(context: dict) -> FriendlyError:
        ...
        return FriendlyError(
            title="No Faces Detected", message=message, suggestions=suggestions
        )
    ...
    def _handle_permission_error(context: dict) -> FriendlyError:
        ...
        return FriendlyError(
            title="Permission Denied", message=message, suggestions=suggestions
        )
    ...
    def _handle_invalid_mapping_error(context: dict) -> FriendlyError:
        ...
        return FriendlyError(
            title="Invalid Face Mapping", message=message, suggestions=suggestions
        )
...
def convert_to_faceoff_error(
    ...
    if "permissionerror" in error_type.lower() or "permission denied" in error_str:
        return FileValidationError(
            f"Permission denied: {error}", context.get("file_path")
        )

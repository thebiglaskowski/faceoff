...
def clear_cuda_caches(
    device_ids: Optional[List[int]] = None, *, force: bool = True
) -> None:
...
class MemoryManager:
    ...
    def clear_cache(self, force: bool = False) -> None:
        ...
        if force or self.should_clear_cache():
            ...
            logger.info(
                "CUDA cache cleared on device %d: freed %.2f MB (%.1f%% → %.1f%% utilization)",
                self.device_id,
                freed_mb,
                stats_before["utilization_pct"],
                stats_after["utilization_pct"],
            )

    def get_optimal_batch_size(
        self, current_batch_size: int, available_vram_mb: Optional[float] = None
    ) -> int:
        """
        ...
        logger.info(
            "Reducing batch size: %d → %d (available VRAM: %.0f MB)",
            current_batch_size,
            max_batch_size,
            available,
        )
    ...
    def handle_oom_error(self, current_batch_size: int) -> Tuple[int, bool]:
        ...
        if new_batch_size < self.min_batch_size:
            logger.error(
                "Batch size cannot be reduced below minimum (%d) - OOM unrecoverable",
                self.min_batch_size,
            )
            return self.min_batch_size, False

        logger.info(
            "Reducing batch size for OOM recovery: %d → %d",
            current_batch_size,
            new_batch_size,
        )
    ...
    def log_memory_stats(self, prefix: str = "") -> None:
        ...
        stats = self.get_memory_stats()
        logger.info(
            "%sGPU Memory (device %d): %.0f/%.0f MB allocated (%.1f%%), %.0f MB free",

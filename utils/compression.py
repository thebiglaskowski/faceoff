...
def _calculate_timeout(
    file_size_bytes: int, base_timeout: int = BASE_TIMEOUT_SECONDS
) -> int:
    """
...
def compress_image(
    input_path: str, output_path: Optional[str] = None, quality: int = 95
) -> Tuple[bool, str, dict]:
    """
    ...
    try:
        ...
        logger.info(
            f"Compressed image: {input_file.name} | {original_size:,} -> {new_size:,} bytes ({savings_percent:.1f}% reduction)"
        )

        return (
            True,
            f"✅ Compressed: {savings_percent:.1f}% smaller ({_format_bytes(original_size)} -> {_format_bytes(new_size)})",
            stats,
        )
...
def compress_gif(
    input_path: str, output_path: Optional[str] = None, lossy: int = 30
) -> Tuple[bool, str, dict]:
    """
    ...
    try:
        ...
        logger.debug(
            f"GIF compression timeout: {timeout}s for {_format_bytes(original_size)} file"
        )
        ...
        if gifsicle_path:
            try:
                ...
                if result.returncode == 0 and temp_output.exists():
                    ...
                    logger.info(
                        f"Compressed GIF (gifsicle): {input_file.name} | {original_size:,} -> {new_size:,} bytes ({savings_percent:.1f}% reduction)"
                    )
                ...
                logger.warning(
                    f"gifsicle: timed out after {timeout}s, falling back to ImageMagick/PIL"
                )
            except Exception as e:
        ...
        if shutil.which("magick"):
            try:
                ...
                cmd = [
                    ...
                    "-layers",
                    "optimize",  # Optimize frame overlaps
                    "-fuzz",
                    "5%",  # Allow slight color variations for better compression
                    str(temp_output),
                ]
                ...
                if result.returncode == 0 and temp_output.exists():
                    ...
                    logger.info(
                        f"Compressed GIF (ImageMagick): {input_file.name} | {original_size:,} -> {new_size:,} bytes ({savings_percent:.1f}% reduction)"
                    )
                ...
                logger.warning(
                    f"ImageMagick: timed out after {timeout}s, falling back to PIL"
                )
            except Exception as e:
        ...
        if frames:
            ...
            logger.info(
                f"Compressed GIF (PIL): {input_file.name} | {original_size:,} -> {new_size:,} bytes ({savings_percent:.1f}% reduction)"
            )
...
def compress_video(
    input_path: str, output_path: Optional[str] = None, crf: int = 23
) -> Tuple[bool, str, dict]:
    """
    ...
    try:
        ...
        try:
            ...
            if result.returncode == 0 and output_file.exists():
                ...
                logger.info(
                    f"Compressed video: {input_file.name} | {original_size:,} -> {new_size:,} bytes ({savings_percent:.1f}% reduction)"
                )
...
def should_skip_compression(file_path: str, media_type: str) -> bool:
    ...
    if media_type == "video" and config.get(
        "compression", "skip_optimized_video", default=True
    ):
        try:
...
def compress_media_file(
    file_path: str, media_type: str, **kwargs
) -> Tuple[bool, str, dict]:
    """

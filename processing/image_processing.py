...
def _get_restorer(restoration_model: str, gpu_id: int, restoration_weight: float = 0.5):
    ...
    if restoration_model == "CodeFormer":
        ...
        return restorer, lambda img, w: restorer.restore_faces_in_frame(img, fidelity_weight=w)
    else:
        # Default to GFPGAN
...
def _apply_enhancement(
    ...
    if enhancement_model == "SwinIR":
        from processing.swinir_enhancement import enhance_image_swinir_file, SWINIR_MODELS
...
def process_image(
    ...
    if restore_faces:
        ...
        restorer, restore_fn = _get_restorer(restoration_model, gpu_id, restoration_weight)
        try:
    ...
    if enhance:
        ...
        enhanced_path = _apply_enhancement(
            output_path, output_dir, enhancement_model, model_name,
            tile_size, outscale, gpu_id, denoise_strength, use_fp32, pre_pad
        )

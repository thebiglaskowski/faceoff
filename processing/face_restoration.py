...
class FaceRestorer:
        ...
        try:
            ...
            if not model_path.exists():
                ...
                try:
                    ...
                    logger.error(
                        "Failed to download GFPGAN model from %s: %s",
                        url,
                        download_error,
                    )
                    raise RuntimeError(
                        f"Could not download GFPGAN model. Please check your internet connection or manually download from {url} to {model_path}"
                    )
            ...
            logger.info(
                "GFPGAN v%s initialized on device %d",
                self.model_version,
                self.device_id,
            )
    ...
    def restore_face(
        self, image: np.ndarray, has_aligned: bool = False, weight: float = 0.5
    ) -> np.ndarray:
        """
    ...
    def restore_faces_in_frame(
        self, frame: np.ndarray, weight: float = 0.5
    ) -> np.ndarray:
        """
...
def restore_face_batch(
    images: list, device_id: int = 0, weight: float = 0.5, model_version: str = "1.4"
) -> list:
    """

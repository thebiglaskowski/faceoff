...
# einops removed as a dependency – the single usage below is replaced with native reshape+permute
...
class OCAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi):
        ...
        # einops rearrange('b (nc ch owh oww) nw -> nc (b nw) (owh oww) ch')
        kv_windows = (
            kv_windows.view(b, 2, c, self.overlap_win_size, self.overlap_win_size, -1)
            .permute(1, 0, 5, 3, 4, 2)
            .contiguous()
            .view(
                2,
                b * kv_windows.shape[2],

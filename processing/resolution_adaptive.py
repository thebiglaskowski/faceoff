...
class ResolutionAdaptiveProcessor:
    ...
    def __init__(self, detection_scale: float = 0.5, min_resolution: int = 640):
        ...
        logger.info(
            "Resolution-adaptive processing enabled (scale=%.2f)", self.detection_scale
        )
    ...
    def detect_faces_adaptive(self, processor, frame: np.ndarray) -> List:
        """
    ...
    def extract_face_region(
        self, frame: np.ndarray, face, padding: float = 0.3
    ...
    def paste_face_region(
        ...
        if region_height != (y2 - y1) or region_width != (x2 - x1):
            face_region = cv2.resize(
                face_region, (x2 - x1, y2 - y1), interpolation=cv2.INTER_LINEAR
            )
        ...
        if blend_edges:
            ...
            if feather > 0:
                ...
                # Blend using mask
                result[y1:y2, x1:x2] = (
                    face_region * mask + result[y1:y2, x1:x2] * (1 - mask)
                ).astype(np.uint8)
            else:
    ...
    if face_mappings:
        for src_idx, dst_idx in face_mappings:
            if src_idx < len(src_faces) and dst_idx < len(dst_faces):
                ...
                result = processor.swapper.get(
                    result, face, src_faces[src_idx], paste_back=True
                )
    else:
        ...
        for face in dst_faces:
            if src_faces:
                result = processor.swapper.get(
                    result, face, src_faces[0], paste_back=True
                )

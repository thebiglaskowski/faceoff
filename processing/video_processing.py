        ...
        if face_mappings:
            for src_idx, dst_idx in face_mappings:
                if (
                    ...
                    swapped = gpu_instance.swap_face(
            ...
            for face in dst_faces:
                if src_faces and face is not None:
                    swapped = gpu_instance.swap_face(
                        swapped, face, src_faces[0], paste_back=True
                    )
...
def process_frames_batch(
    ...
    if adaptive_processor:
        all_dst_faces = [
            adaptive_processor.detect_faces_adaptive(processor, frame)
            for frame in frames
        ]
    else:
    ...
    for frame, dst_faces in zip(frames, all_dst_faces):
        ...
        if face_mappings:
            ...
            # Default: swap first source to all destination faces (batched ONNX)
            valid_faces = [f for f in dst_faces if f is not None]
            if valid_faces:
                swapped = processor.swap_face_batch(frame, valid_faces, src_faces[0])
...
def process_video(
    ...
    # Load video metadata and extract audio
    ...
    frame_dir = temp_manager.get_temp_dir("video") / f"frames_{Path(dest_path).stem}"
    frame_results = video_io.extract_video_frames(dest_path, frame_dir)
    frames = [
        cv2.cvtColor(cv2.imread(str(fr.path)), cv2.COLOR_BGR2RGB)
        for fr in frame_results
    ]
    logger.info(
        "inswapper-shape: %s, total frames: %d",
        processor.swapper.input_shape,
        len(frames),
    )
    ...
    if adaptive_detection and detection_scale is not None:
        adaptive_processor = ResolutionAdaptiveProcessor(
            detection_scale=detection_scale
        )
        logger.info(
            "Resolution-adaptive detection enabled (scale=%.2f)", detection_scale
        )

    # Multi-GPU setup - using model pool for isolated ONNX sessions per GPU
    if len(device_ids) > 1:
        logger.info(
            "Multi-GPU Video Mode: %d GPUs: %s (using model pool)",
            len(device_ids),
            device_ids,
        )
        ...
        logger.info(
            "Source faces detected (after filtering): %d", len(processor_src_faces[0])
        )
        ...
        logger.info(
            "Processing %d frames across %d GPUs in batches of %d...",
            len(frames),
            num_gpus,
            batch_size,
        )
        ...
        with ThreadPoolExecutor(max_workers=num_gpus) as executor:
            futures = [
                executor.submit(process_gpu_frames_with_pool, gpu_idx)
        ...
        if adaptive_detection:
            logger.info(
                "Resolution-adaptive processing enabled (scale=%.2f)", detection_scale
            )
            adaptive_processor = ResolutionAdaptiveProcessor(
                detection_scale=detection_scale
            )
        ...
        if use_async_pipeline and len(frames) > 10:
            ...
            logger.info(
                "Processing %d frames in batches of %d...", len(frames), batch_size
            )

            with progress.track(
                len(frames), "Processing video frames", "frame", "Face Swapping"
            ) as pbar:
    ...
    if restore_faces:
        ...
        if restoration_model == "CodeFormer":
            ...
            restore_fn = lambda frame, w: restorer.restore_faces_in_frame(
                frame, fidelity_weight=w
            )
        else:
    ...
    # Write video using FFmpeg via video_io
    if has_audio:
        audio_path = video_io.extract_audio(dest_path, output_dir)
        if audio_path:
            logger.info("Audio track extracted for video output.")
        else:
            audio_path = None
            logger.warning("Failed to extract audio track from the original video.")
    else:
        audio_path = None
        logger.warning("Processed video will not contain audio.")

    success = video_io.write_video_from_frames(
        processed_frames,
        output_path,
        fps,
    ...
    if not success:
        logger.error(
            "Failed to write video via video_io, attempting direct ffmpeg fallback..."
        )
        # Fallback: Save frames to temp directory and use FFmpeg directly
        temp_manager = get_temp_manager()
        temp_frames_dir = (
            temp_manager.get_temp_dir("video") / f"fallback_{Path(dest_path).stem}"
        )
    ...
    if enhance:
        ...
        temp_manager = get_temp_manager()
        enhancement_temp_dir = (
            temp_manager.get_temp_dir("video") / f"enhance_{Path(output_path).stem}"
        )
        ...
        try:
            ...
            video_io.extract_video_frames(str(output_path), temp_frames_dir)
            logger.info("Extracting frames from video for enhancement")

            frame_paths = sorted(temp_frames_dir.glob("frame_*.png"))
            ...
            if result:
                ...
                enhanced_frames, output_path = result
                frame_count = (
                    len(enhanced_frames) if isinstance(enhanced_frames, list) else 1
                )
                logger.info(
                    "Enhanced video saved to %s with %d frames",
                    output_path,
                    frame_count,
                )
            else:
            ...
            try:
                if enhancement_temp_dir.exists():
                    ...
                    logger.info(
                        "Cleaned up enhancement temp directory: %s",
                        enhancement_temp_dir.name,
                    )
            except Exception as e:

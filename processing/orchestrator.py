...
def process_media(opts: ProcessOptions) -> Tuple[Optional[str], Optional[str]]:
    ...
    if len(device_ids) > 1 and opts.media_type in ["video", "gif"]:
        if config.multi_gpu_video_enabled:
            logger.info(
                "Multi-GPU enabled for %s with %d GPUs (using model pool)",
                opts.media_type.upper(),
                len(device_ids),
            )
        else:
            logger.info(
                "Multi-GPU disabled in config for %s. Using single GPU: %d",
                opts.media_type.upper(),
                device_ids[0],
            )
            device_ids = [device_ids[0]]
    ...
    logger.info(
        "Initializing MediaProcessor with device(s): %s (TensorRT auto-detected)",
        device_ids,
    )
    ...
    try:
        ...
        if opts.media_type == "image":
            result_path, video_path = process_image(
                ...
                opts.tile_size,
                opts.outscale,
                opts.face_confidence,
                device_ids,
                opts.face_mappings,
                opts.model_name,
                opts.denoise_strength,
                opts.use_fp32,
                opts.pre_pad,
                opts.restore_faces,
                opts.restoration_weight,
                opts.enhancement_model,
                opts.restoration_model,
            )
        elif opts.media_type == "video":
        ...
        logger.info(
            f"Processing returned: result_path={result_path}, video_path={video_path}"
        )
        ...
        if output_to_compress:
            try:
                ...
                success, message, stats = compress_media_file(
                    output_to_compress, opts.media_type
                )
                if success:

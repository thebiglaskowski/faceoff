...
warnings.filterwarnings(
    "ignore", message=r".*`rcond` parameter will change.*", category=FutureWarning
)
warnings.filterwarnings(
    "ignore", message=r".*parameter 'pretrained' is deprecated.*", category=UserWarning
)
warnings.filterwarnings(
    "ignore", message=r".*Arguments other than a weight enum.*", category=UserWarning
)
...
from ui.components.gallery_tab import (
    create_gallery_tab,
    refresh_gallery,
    update_gallery,
)
...
from ui.helpers.face_detection import (
...
from ui.helpers.preview import show_gif_preview, validate_target_video_upload
...
def toggle_enhancement_controls(enabled):
    ...
    ] * 4  # model_row, enhancement_options_row, enhancement_advanced_row, enhancement_model_selector
...
def show_delete_controls(evt: gr.SelectData, media_type_display: str):
    ...
    if isinstance(evt.value, dict):
        ...
        file_path = (
            evt.value.get("image", {}).get("path")
            if isinstance(evt.value.get("image"), dict)
...
def delete_and_refresh(file_path: str, media_type_display: str, limit: str = "24"):
    ...
    logger.info(
        f"Delete triggered - file_path: '{file_path}', media_type: '{media_type_display}'"
    )
    ...
    return (
        ...
        gr.update(
            value="", visible=False
        ),  # processing_settings_display (hide after delete)
    )
...
def toggle_restoration_controls(enabled):
    ...
    return [
        gr.update(visible=enabled)
    ] * 2  # restoration_row, restoration_model_selector


def toggle_denoise_slider(model_choice):
    """Show/hide denoise slider based on model selection."""
    supports_denoise = MODEL_OPTIONS.get(model_choice, {}).get(
...
def create_app():
    ...
    with gr.Blocks(title="FaceOff - Face Swapper") as demo:
        ...
        with gr.Accordion("🎨 Preset Management", open=False):
            gr.HTML(
                '<div class="face-swap-box"><strong>💾 Save & Load Settings</strong><br/>Manage your favorite face-swap configurations for quick access</div>'
            )
                ...
                with gr.Column(scale=1):
                    with gr.Row():
                        load_preset_btn = gr.Button(
                            "📥 Load Preset", variant="primary", size="sm"
                        )
                        delete_preset_btn = gr.Button(
                            "🗑️ Delete Preset", variant="secondary", size="sm"
                        )
                    ...
                    save_preset_btn = gr.Button(
                        "💾 Save as Preset", variant="primary", size="sm"
                    )

                    preset_status = gr.Textbox(
                        label="Status", value="", interactive=False, max_lines=1
                    )
        ...
        with gr.Accordion("🖥️ GPU Status Monitor", open=False):
            gr.HTML(
                '<div class="face-swap-box"><strong>⚡ System Performance</strong><br/>Monitor GPU memory usage and allocation</div>'
            )
            ...
            with gr.Row():
                for idx, info in enumerate(initial_gpu_info):
                    gpu_textbox = gr.Textbox(
                        label=(
                            f"GPU {idx}" if len(initial_gpu_info) > 1 else "GPU Status"
                        ),
                        value=info,
                        lines=4,
                        interactive=False,
        ...
        img_components["enhance_toggle"].change(
            ...
            outputs=[
                ...
                img_components["enhancement_model_selector"],
            ],
        )
        ...
        img_components["restore_faces_toggle"].change(
            ...
            outputs=[
                ...
                img_components["restoration_model_selector"],
            ],
        )
        ...
        img_components["detect_faces_btn"].click(
            detect_faces_for_mapping,
            inputs=[
                img_components["source_img"],
                img_components["target_img"],
                img_components["face_confidence"],
            ],
            outputs=[
        ...
        img_components["target_img"].change(
            ...
            outputs=[
                img_components["face_mapping_status"],
                img_components["current_mappings"],
            ],
        )
        ...
        img_components["run_image_btn"].click(
            ...
            inputs=[
                ...
                img_components["face_confidence"],
                img_components["gpu_selection"],
                img_components["model_selector"],
                img_components["denoise_slider"],
                ...
                img_components["tensorrt_fp16_checkbox"],
                img_components["enhancement_model_selector"],
                img_components["restoration_model_selector"],
            ],
        ...
        gif_components["enhance_toggle_gif"].change(
            ...
            outputs=[
                ...
                gif_components["enhancement_model_selector_gif"],
            ],
        )
        ...
        gif_components["restore_faces_toggle_gif"].change(
            ...
            outputs=[
                ...
                gif_components["restoration_model_selector_gif"],
            ],
        )
        ...
        gif_components["source_gif"].change(
            detect_faces_simple,
            inputs=[
                gif_components["source_gif"],
                gif_components["face_confidence_gif"],
            ],
        ...
        gif_components["detect_faces_btn_gif"].click(
            ...
            inputs=[
                gif_components["source_gif"],
                gif_components["target_gif_file"],
                gif_components["face_confidence_gif"],
            ],
            outputs=[
        ...
        gif_components["run_gif_btn"].click(
            ...
            inputs=[
                ...
                gif_components["face_confidence_gif"],
                gif_components["gpu_selection_gif"],
                gif_components["model_selector_gif"],
                gif_components["denoise_slider_gif"],
                ...
                gif_components["tensorrt_fp16_checkbox_gif"],
                gif_components["enhancement_model_selector_gif"],
                gif_components["restoration_model_selector_gif"],
            ],
        ...
        vid_components["enhance_toggle_vid"].change(
            ...
            outputs=[
                ...
                vid_components["enhancement_model_selector_vid"],
            ],
        )
        ...
        vid_components["restore_faces_toggle_vid"].change(
            ...
            outputs=[
                ...
                vid_components["restoration_model_selector_vid"],
            ],
        )
        ...
        vid_components["source_vid"].change(
            detect_faces_simple,
            inputs=[
                vid_components["source_vid"],
                vid_components["face_confidence_video"],
            ],
        ...
        vid_components["run_video_btn"].click(
            ...
            inputs=[
                ...
                vid_components["face_confidence_video"],
                vid_components["gpu_selection_video"],
                vid_components["model_selector_vid"],
                vid_components["denoise_slider_vid"],
                ...
                vid_components["tensorrt_fp16_checkbox_vid"],
                vid_components["enhancement_model_selector_vid"],
                vid_components["restoration_model_selector_vid"],
            ],
        ...
        save_preset_btn.click(
            ...
            fn=lambda name: "", inputs=[preset_name_input], outputs=[preset_name_input]
        )
        ...
        # GPU refresh handler
        refresh_gpu_btn.click(fn=refresh_gpu_info, outputs=gpu_textboxes)

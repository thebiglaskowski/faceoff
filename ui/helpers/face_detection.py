...
def _get_ui_processor() -> MediaProcessor:
    ...
    if _ui_processor is None:
        ...
        logger.debug(
            "UI face-detection processor initialized (TensorRT=%s)",
            config.tensorrt_enabled,
        )
    else:
...
def extract_face_thumbnails(
    image_rgb: "np.ndarray", faces_raw: List, with_index: bool = True
) -> List[Image.Image]:
    """
...
def detect_faces_for_mapping(
    source_img: Image.Image, target_img: Image.Image, face_confidence: float
    ...
    try:
        ...
        return (
            ...
            status,
            gr.update(
                choices=src_choices, value=src_choices[0] if src_choices else None
            ),
            gr.update(
                choices=tgt_choices, value=tgt_choices[0] if tgt_choices else None
            ),
    ...
    try:
        ...
        if target_path.lower().endswith(".gif"):
            ...
            first_frame_rgb = (
                frames[0]
                if frames
                else np.array(Image.open(target_path).convert("RGB"))
            )
        else:
            with video_io.StreamingFrameReader(target_path, hwaccel=False) as reader:
                frames = reader.read_chunk(1)
            first_frame_rgb = (
                frames[0] if frames else np.zeros((64, 64, 3), dtype=np.uint8)
            )
        ...
        tgt_faces = extract_face_thumbnails(
            first_frame_rgb, tgt_faces_raw, with_index=True
        )

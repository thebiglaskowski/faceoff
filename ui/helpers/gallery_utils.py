...
def get_media_files(
    output_dir: str, media_type: str, max_files: int = 50
) -> List[Tuple[str, str]]:
    """
    ...
    if not _should_refresh_cache(media_type, media_path):
        ...
        logger.info(
            f"Using cached data for {media_type} ({len(cached_files)} files, returning {min(len(cached_files), max_files)})"
        )
    ...
    try:
        ...
        if total_count > max_files:
            logger.info(
                f"Showing {max_files} of {total_count} {media_type} files (most recent)"
            )
        else:
...
def get_image_files(
    output_dir: str = "outputs", max_files: int = 50
) -> List[Tuple[str, str]]:
